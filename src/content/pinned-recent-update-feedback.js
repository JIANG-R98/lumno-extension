(function(root, factory) {
  ...
  function createFeedbackController(options) {
    ...
    const breatheDuration = Math.max(0, Number(config.previewTimings && config.previewTimings.breathe) || 1050);
    const baseTimings = {
    ...
    function ensureSurface() {
      ...
      style.textContent = `
        ...
        .surface[data-visual-variant="homepage-card"] .incoming-footer { display:flex; align-items:center; gap:8px; width:auto; min-width:0; margin:10px 5px 0 10px; color:rgba(0,0,0,.51); font-size:14px; font-weight:400; line-height:1.2; }
        ...
        .confetti { display:none; position:absolute; inset:0; z-index:5; overflow:visible; pointer-events:none; }
        .surface[data-celebrate="true"][data-phase="success"] .confetti { display:block; }
        .confetti-piece { --mx:0px; --x:0px; --lift:82px; --fall:130px; --r:180deg; --delay:0ms; position:absolute; left:50%; top:-12px; width:6px; height:12px; border-radius:2px; background:#14a0fd; opacity:0; animation:confetti-burst var(--lumno-flow-confetti,1370ms) cubic-bezier(.16,.78,.3,1) var(--delay) both; }
        ...
        @media (prefers-color-scheme:dark) { .panel { color:#edf7ff;background:linear-gradient(135deg,rgba(30,41,59,.86),rgba(15,23,42,.82)); } .feedback-backdrop{background:radial-gradient(ellipse at center,rgba(18,27,36,.82) 0%,rgba(18,27,36,.48) 46%,rgba(18,27,36,.12) 70%,transparent 84%)} .record{background:rgba(133,205,254,.07)} .record-title,.card-site{color:#edf7ff}.record-label,.record-url,.card-title,.card-footer{color:#9bb5c8}.card-inner{background:rgba(20,31,42,.82)} .surface[data-visual-variant="homepage-card"] .home-card,.surface[data-visual-variant="homepage-card"] .incoming-card{background:#1c2228;border-color:rgba(255,255,255,.08);box-shadow:0 18px 30px rgba(0,0,0,.38),0 8px 16px rgba(0,0,0,.24)} .surface[data-visual-variant="homepage-card"] .card-inner,.surface[data-visual-variant="homepage-card"] .incoming-inner{background:rgba(22,22,22,.72);border-color:rgba(255,255,255,.08);box-shadow:0 3px 8px rgba(0,0,0,.22),0 2px 4px rgba(0,0,0,.12);backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px)} .surface[data-visual-variant="homepage-card"] .incoming-inner>.card-title,.surface[data-visual-variant="homepage-card"] .incoming-inner .card-site{color:#e5e7eb} .surface[data-visual-variant="homepage-card"] .incoming-footer{color:rgba(226,232,240,.75)} .surface[data-visual-variant="homepage-card"] .incoming-card::before{background:radial-gradient(ellipse at center,rgba(47,103,151,.34),rgba(47,103,151,.1) 62%,transparent 76%)} .x-lumno-action-button--secondary{background:#1e293b;color:#f8fafc;border-color:rgba(148,163,184,.18)} }
        @media (prefers-reduced-motion:reduce) { .glow,.feedback-backdrop,.panel,.record,.card-content,.home-card,.incoming-card,.confetti-piece,.undo-toast,.actions { animation-duration:1ms!important;transition-duration:1ms!important; } .confetti{display:none!important} }
